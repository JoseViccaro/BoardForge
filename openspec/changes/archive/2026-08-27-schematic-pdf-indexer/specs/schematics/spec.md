# OpenSpec Delta Specification: `schematics` (Extension: Vector Schematic Indexing & Cross-Probing)

**Change ID:** `schematic-pdf-indexer`  
**Domain:** `schematics`  
**Standard:** RFC 2119 (MUST, SHALL, SHOULD, MAY)  
**Target Entities & Modules:** `SchematicDocument`, `SchematicPage`, `SchematicSymbol`, `MultiPageSymbolAggregate`, `SchematicPinLocation`, `BoundingBox2D`, `SchematicSpatialIndex`, `SymbolExtractorService`, `SchematicCrossProbeIndex`, `VectorToken`, `ISchematicParser`

---

## 1. Domain Delta Overview

This delta specification extends the `schematics` domain beyond power sequence state machines and power trees to encompass full **Vector PDF/SVG stream parsing, spatial token indexing (R-Tree / 2D bounding boxes), symbol & net extraction, and bidirectional cross-probing** against BoardView physical CAD models.

---

## 2. Formal Delta Requirements
